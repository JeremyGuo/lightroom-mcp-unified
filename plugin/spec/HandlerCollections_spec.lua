...
describe('Collection hierarchy', function()
    it('passes the requested parent set to Lightroom', function()
        local parent={getName=function() return 'Trips' end,getChildCollections=function() return {} end,getChildCollectionSets=function() return {} end}
        local catalog,h=setup({collectionSets={parent}})
        local actual
        catalog.createCollection=function(_,_,p) actual=p end
        h.createCollection({name='Japan',parent='Trips'})
        assert.are.equal(parent,actual)
        assert.has_error(function() h.createCollection({name='Japan',parent='Missing'}) end)
    end)
    it('resolves full paths and rejects ambiguous leaf names', function()
        local a=helper.fakeCollection('Album',{});local b=helper.fakeCollection('Album',{})
        local parent={getName=function() return 'Trips' end,getChildCollections=function() return {b} end,getChildCollectionSets=function() return {} end}
        local catalog=setup({collections={a},collectionSets={parent}})
        local lookup=require 'CollectionLookup'
        assert.has_error(function() lookup.find(catalog,'Album',false) end)
        assert.are.equal(b,lookup.find(catalog,'Trips / Album',false))
    end)
end)
