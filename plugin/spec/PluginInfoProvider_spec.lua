...
local HANDLER_MODULES = {
    ...
    'HandlerSelection', 'HandlerDevelop', 'HandlerController',
}
...
local function installStubs(prefs, asyncTasks, opts)
    ...
    helper.installImport({
        LrTasks = {
            ...
            sleep = function()
                if opts.stopLoopOnSleep and _G.LightroomMCPUnified_State then
                    _G.LightroomMCPUnified_State.running = false
                end
                if opts.onSleep then opts.onSleep(_G.LightroomMCPUnified_State) end
            end,
            pcall = pcall,
        },
...
describe("PluginInfoProvider lifecycle", function()
    before_each(function()
        _G.LightroomMCPUnified_State = nil
        installStubs()
    end)

    it("creates fresh state on first load", function()
        loadInfoProvider()
        assert.is_not_nil(_G.LightroomMCPUnified_State)
        assert.is_false(_G.LightroomMCPUnified_State.running)
    end)
    ...
    it("preserves a running server across a Plug-in Manager render", function()
        ...
        _G.LightroomMCPUnified_State.running = true
        local sock = { close = function() error("must not close on render") end }
        _G.LightroomMCPUnified_State.requestSocket = sock
        local stateBefore = _G.LightroomMCPUnified_State

        loadInfoProvider()

        assert.are.equal(stateBefore, _G.LightroomMCPUnified_State)
        assert.is_true(_G.LightroomMCPUnified_State.running)
        assert.are.equal(sock, _G.LightroomMCPUnified_State.requestSocket)
    end)
    ...
    it("resetForReload stops a stale running instance", function()
        ...
        local s = _G.LightroomMCPUnified_State
        s.running = true
        s.token = "tok"
...
describe("PluginInit reload reset", function()
    ...
    it("resets a surviving running instance on reload", function()
        ...
        assert.is_false(_G.LightroomMCPUnified_State.running)
        assert.is_nil(_G.LightroomMCPUnified_State.requestSocket)
        assert.is_true(closed)
        -- Transient state returns to fresh-state defaults (Copilot #141).
        assert.are.equal(0, _G.LightroomMCPUnified_State.requestsProcessed)
        assert.is_nil(_G.LightroomMCPUnified_State.lastEvent)
        assert.is_nil(_G.LightroomMCPUnified_State.requestPort)
        assert.is_false(_G.LightroomMCPUnified_State.responseNeedsRebind)
    end)
end)
...
describe("PluginInfoProvider server task", function()
    ...
    describe("undecodable requests", function()
        local function startServerCapturing(sends)
            ...
            local mod = loadInfoProvider()
            mod.startServer()
            _G.LightroomMCPUnified_State.sendConnected = true
            return binds
        end
        ...
        it("answers an authenticated client instead of leaving it to time out", function()
            ...
            binds[1].onMessage(nil, malformed(_G.LightroomMCPUnified_State.token))
        ...
        it("stays silent when no id can be salvaged", function()
            ...
            binds[1].onMessage(nil, '{"hello":"' .. _G.LightroomMCPUnified_State.token .. '","action":"ping"')
    ...
    it("bumps instanceId on every start", function()
        ...
        mod.startServer()
        assert.are.equal(1, _G.LightroomMCPUnified_State.instanceId)
        mod.startServer()
        assert.are.equal(2, _G.LightroomMCPUnified_State.instanceId)
    end)
    ...
    it("ignores a superseded instance's cleanup handler", function()
        ...
        local liveReq = _G.LightroomMCPUnified_State.requestSocket
        local liveResp = _G.LightroomMCPUnified_State.responseSocket
        local liveToken = _G.LightroomMCPUnified_State.token

        -- Old context cleanup fires late (after the new instance rebound).
        staleCleanup()

        assert.are.equal(liveReq, _G.LightroomMCPUnified_State.requestSocket)
        assert.are.equal(liveResp, _G.LightroomMCPUnified_State.responseSocket)
        assert.are.equal(liveToken, _G.LightroomMCPUnified_State.token)
    end)
    ...
    it("tears down its own sockets when not superseded", function()
        ...
        assert.is_nil(_G.LightroomMCPUnified_State.requestSocket)
        assert.is_nil(_G.LightroomMCPUnified_State.responseSocket)
        assert.is_nil(_G.LightroomMCPUnified_State.token)
    end)
    ...
    it("does not churn freshly bound sockets when recovery flags are stale", function()
        ...
        _G.LightroomMCPUnified_State.requestNeedsReconnect = true
        _G.LightroomMCPUnified_State.responseNeedsRebind = true
        _G.LightroomMCPUnified_State.responseNeedsReconnect = true

        mod.startServer()

        assert.are.equal(0, #ops)
        assert.is_false(_G.LightroomMCPUnified_State.requestNeedsReconnect)
        assert.is_false(_G.LightroomMCPUnified_State.responseNeedsRebind)
        assert.is_false(_G.LightroomMCPUnified_State.responseNeedsReconnect)
    end)
end)
...
describe("stale-connection follow-up fixes (PR #151 re-review)", function()
    ...
    it("clears a stale lastRequestTime on a fresh REQUEST connect so it doesn't leak into the new idle clock", function()
        ...
        _G.LightroomMCPUnified_State.lastRequestTime = os.time() - 999
        binds[1].onConnected()

        assert.is_nil(_G.LightroomMCPUnified_State.lastRequestTime)
        assert.is_not_nil(_G.LightroomMCPUnified_State.lastConnectedTime)
    end)
    ...
    it("keeps a request counted in-flight until sendResponse actually completes, not just until the handler returns", function()
        ...
        mod.startServer()
        local state = _G.LightroomMCPUnified_State
...
describe("cooperative shutdown at Lightroom quit (issue 195)", function()
    ...
    it("leaves the sockets for the server task to release", function()
        ...
        assert.is_not_nil(_G.LightroomMCPUnified_State.requestSocket)
        assert.is_not_nil(_G.LightroomMCPUnified_State.responseSocket)
    ...
    it("is a cheap no-op when the server never started", function()
        ...
        assert.are.same({}, ops)
        assert.is_true(_G.LightroomMCPUnified_State.shuttingDown)
    end)
    ...
    it("drops the response of a handler that finished after quit began", function()
        ...
        package.loaded.HandlerSearch = {
            searchPhotos = function()
                _G.LightroomMCPUnified_State.shuttingDown = true
                return { photos = {} }
            end,
        }
        local mod = loadInfoProvider()
    ...
    it("abandons a pending stale-connection restart", function()
        ...
        mod.startServer()
        local instanceBefore = _G.LightroomMCPUnified_State.instanceId
        _G.LightroomMCPUnified_State.shuttingDown = true
        local pendingRestart = asyncTasks[1]
        assert.is_function(pendingRestart)
        pendingRestart()

        assert.are.equal(instanceBefore, _G.LightroomMCPUnified_State.instanceId)
    end)
    ...
    it("lets the Plug-in Manager Start button supersede an earlier shutdown", function()
        ...
        assert.is_false(_G.LightroomMCPUnified_State.shuttingDown)
        assert.is_not_nil(_G.LightroomMCPUnified_State.token)
    end)
