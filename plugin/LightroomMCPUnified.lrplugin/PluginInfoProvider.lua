...
local HandlerController = require 'HandlerController'
local Log = require 'Log'

local DEFAULT_REQUEST_PORT = 58773
local DEFAULT_RESPONSE_PORT = 58774
...
if not _G.LightroomMCPUnified_State then
...
local function tokenFilePath()
    return LrPathUtils.child(LrPathUtils.child(tokenDir(), "lightroom-mcp-unified"), "token")
end
...
local function generateToken()
    -- Two UUIDs provide a long per-user bearer token; do not log it.
...
local function writeTokenFile(token)
    local dir = LrPathUtils.child(tokenDir(), "lightroom-mcp-unified")
    ...
    -- The Node bridge restricts the token file to its owner on POSIX.
    -- Start the bridge once before using the plugin on a shared machine.
    return true
end
...
local DISPATCH = {
    ...
    lr_ping = HandlerController.ping,
    lr_capabilities = HandlerController.capabilities,
    lr_get_settings = HandlerController.getSettings,
    lr_apply_settings = HandlerController.applySettings,
    lr_batch_apply_settings = HandlerController.batchApplySettings,
    lr_auto_tone = HandlerController.autoTone,
    lr_reset = HandlerController.reset,
    lr_export_preview = HandlerController.exportPreview,
    lr_crop = HandlerController.crop,
    lr_add_mask = HandlerController.addMask,
    lr_update_mask = HandlerController.updateMask,
    lr_lens_blur = HandlerController.lensBlur,
    lr_enhance = HandlerController.enhance,
}
...
local function dispatchAction(request)
    ...
    -- Serialize catalog/UI operations across callers, but let heartbeats through.
    if not isHeartbeat then
        local waited = 0
        while pluginState.dispatchBusy and waited < 300 do
            if request.expires_at and os.time() >= request.expires_at then break end
            LrTasks.sleep(0.05)
            waited = waited + 0.05
        end
        if pluginState.dispatchBusy or (request.expires_at and os.time() >= request.expires_at) then
            sendResponse({ id=id, error="Request expired before execution; no changes made" })
            pluginState.inFlightRequests = math.max(0, pluginState.inFlightRequests - 1)
            return
        end
        pluginState.dispatchBusy = true
    end
    ...
    if not isHeartbeat then pluginState.dispatchBusy = false end
    if execOk then
...
local function consumeMessage(message)
    ...
    if not parsedOk or type(request) ~= "table" then
        addLog("JSON decode failed (payload omitted to protect auth token)")
...
local function resetForReload()
    ...
    pluginState.dispatchBusy = false
end
    ...
    return {
        {
            ...
            f:row {
                ...
                f:static_text { title = "(default 58773)" },
            },
            f:row {
                ...
                f:static_text { title = "(default 58774)" },
            },
            f:static_text {
