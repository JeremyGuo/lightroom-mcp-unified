...
function CollectionsHandler.createCollection(args)
    ...
    local parent = args.parent and CollectionLookup.find(catalog, args.parent, true) or nil
    if args.parent and not parent then error('Parent collection set not found: ' .. args.parent) end
    ...
    for _, collection in ipairs((parent or catalog):getChildCollections()) do
    ...
    catalog:withWriteAccessDo("Create Collection", function()
        catalog:createCollection(collectionName, parent)
...
local function findCollection(catalog, name)
    return CollectionLookup.find(catalog, name, false)
end
