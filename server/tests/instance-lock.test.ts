...
describe("instance lock", () => {
  ...
  it("rejects a second live bridge for the same ports", () => {
    locks.push(acquireInstanceLock(58773, 58774, baseDir()));

    expect(() => acquireInstanceLock(58773, 58774, baseDir())).toThrow(
  ...
  it("replaces a stale lock with a fully written live lock", () => {
    const dir = baseDir();
    const lockFile = path.join(dir, "bridge-58773-58774.lock");
    fs.writeFileSync(lockFile, "999999999\n");

    locks.push(acquireInstanceLock(58773, 58774, dir));
  ...
  it("allows release after the lock file is already gone", () => {
    const dir = baseDir();
    const lockFile = path.join(dir, "bridge-58773-58774.lock");
    const lock = acquireInstanceLock(58773, 58774, dir);
  ...
  it("removes process handlers when released", () => {
    ...
    const lock = acquireInstanceLock(58773, 58774, baseDir());
