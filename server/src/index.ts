...
const ACTION_TIMEOUTS_MS: Record<string, number> = {
  ...
  lr_export_preview: LONG_RUNNING_TIMEOUT_MS,
  lr_batch_apply_settings: LONG_RUNNING_TIMEOUT_MS,
  lr_enhance: LONG_RUNNING_TIMEOUT_MS,
...
async function main() {
  ...
  try {
    acquireInstanceLock(REQUEST_PORT, RESPONSE_PORT, process.env.LIGHTROOM_MCP_STATE_DIR);
  } catch (err) {
    console.error((err as Error).message);
    process.exit(1);
  }

  if (process.env.LIGHTROOM_MCP_SKIP_PLUGIN_INSTALL !== "1") {
...
function runInstallPlugin(): void {
  ...
  if (!source) {
    console.error("Could not locate bundled LightroomMCPUnified.lrplugin folder near this binary.");
