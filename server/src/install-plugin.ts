...
export function isPluginInstalledAnywhere(): boolean {
  const targets = [
    path.join(lightroomModulesDir(), "LightroomMCPUnified.lrplugin", "Info.lua"),
    path.join(lightroomPluginsDir(), "LightroomMCPUnified.lrplugin", "Info.lua"),
  ];
...
export function findBundledPlugin(startDir: string): string | null {
  const candidates = [
    path.join(startDir, "LightroomMCPUnified.lrplugin"),
    path.join(startDir, "..", "LightroomMCPUnified.lrplugin"),
    path.join(startDir, "..", "..", "LightroomMCPUnified.lrplugin"),
    path.join(startDir, "..", "..", "..", "LightroomMCPUnified.lrplugin"),
    path.join(startDir, "..", "plugin", "LightroomMCPUnified.lrplugin"),
    path.join(startDir, "..", "..", "plugin", "LightroomMCPUnified.lrplugin"),
    path.join(startDir, "..", "..", "..", "plugin", "LightroomMCPUnified.lrplugin"),
  ];
...
export function installPlugin(opts: {
  ...
  const destination = path.join(destDir, "LightroomMCPUnified.lrplugin");
