import { DEVELOP_TOOL_CONTRACTS } from "./develop-tool-contracts.js";
import type { Tool } from "@modelcontextprotocol/server";

type InputSchema = Tool["inputSchema"];
type Schema = NonNullable<InputSchema["properties"]>[string];
 ...
 */
const photoIdSchema = (description: string): Schema => ({
  oneOf: [{ type: "string", minLength: 1 }, { type: "number" }],
  description,
});

const photoIdArray = (description: string): Schema => ({
  type: "array",
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        copy_to: {
          type: "string",
          description: "Copy files into this folder before import. Flat filenames; collisions are rejected. Original files remain untouched.",
        },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        format: {
          type: "string",
          description: "Export format (jpeg, tiff, original)",
          enum: ["jpeg", "tiff", "original"],
        },
        quality: {
          type: "number",
        ...
        width: { type: "number", description: "Max width in pixels; when used alone, constrains the long edge" },
        height: { type: "number", description: "Max height in pixels; when used alone, constrains the long edge" },
        on_existing: {
          type: "string",
          description:
