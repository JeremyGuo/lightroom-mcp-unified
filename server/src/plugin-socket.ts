...
export class PluginSocket {
  ...
  connect(): void {
    ...
    sock.on("data", (chunk: string) => {
      ...
      this.buffer += chunk;
      // A 2048px JPEG is capped at 8 MiB before base64 encoding. Bound a
      // missing-newline/malicious peer too; never allow an unbounded buffer.
      if (Buffer.byteLength(this.buffer, "utf8") > 12 * 1024 * 1024) {
        this.log(`[${this.label}] response frame exceeds 12 MiB; disconnecting`);
        sock.destroy();
        return;
      }
      let idx: number;
